...
*/
	...
	// Uses main get value method to return first char from value - IE value is only one char long
	static char getKeyValueCharWithKey(const byte * data, const char * key);
	...
	// Uses Main buildKeyValue.. Method to create an one char long value
	static bool buildKeyValueDataFromKeyValueChar(byte * data, const char * key, const char value);
